...
Algorithm and Code by Martin Porter (Modified a bit to integrate in the current system)

(More info: https://tartarus.org/martin/PorterStemmer/)
...
static void stemfile(FILE * f, FILE * out)
{  while(TRUE)
   {  int ch = getc(f);
      if (ch == EOF) return;
      if (LETTER(ch))
...
{  
   ...
   //output file
   out = fopen("porter_output.txt", "w");

   s = (char *) malloc(i_max+1);
   
   //input file
   FILE * f = fopen("swordsremoved.txt","r");
